...
**`update.bat` をダブルクリックするだけ。** 最新版をGitHubから取得して入れ替えます。

- 登録済みのデータ（`~/.second_brain/brain.db`）は触りません
- `SOT21_Controller/config.json` の設定も残します
- 起動中の黒い画面がある場合は、先に閉じてから実行してください
