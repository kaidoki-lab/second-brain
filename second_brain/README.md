...
`PROJECT:` 行で区切った文章を `/bulk` に貼ると、企画ごとに振り分けて登録する。
企画が無ければその場で作る。

```
PROJECT: 企画の名前 | ACTIVE | ひとこと説明
...
`PROJECT:` より前に書いた行は「企画の指定なし」として集計され、行き先の企画名を
入力したときだけ取り込む。他のAIが覚えている内容をまとめて移すときに使う。

### 私について（全AI共通の前提）

企画に属さない、依頼主自身の情報。ChatGPTやClaudeが覚えている「あなたについての記憶」を
ここへ集約すると、**どの役割のコンテキストにも毎回入る**。

1. AIに `私について記憶していることを、1行に1件ずつ、箇条書きで全部書き出してください。`
   と聞く
2. その回答を `/profile` に貼り付ける → 不要な行のチェックを外す → 保存

- トークン上限が厳しいときも削られない（`project` や `current_phase` と同格の扱い）
- AIへ渡るのは優先度の高い上位12件まで
- `GET /api/profile` と MCP の `get_profile` でも取得できる
- パスワードや住所などの機微情報は入れないこと（全AIへ渡るため）
